...
namespace Vela.Sqlite;

/// <summary>SQLite database handle for Vela programs.</summary>
public sealed class VelaSqliteDatabase : IDisposable
{
    private SqliteConnection? _connection;
    private bool _disposed;

    private VelaSqliteDatabase(SqliteConnection connection) => _connection = connection;

    /// <summary>Opens a SQLite database at <paramref name="path"/> (file path or <c>:memory:</c>).</summary>
    public static VelaSqliteDatabase Open(string path)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);
        var connectionString = path.Contains('=') ? path : $"Data Source={path}";
        var connection = new SqliteConnection(connectionString);
        connection.Open();
        return new VelaSqliteDatabase(connection);
    }
...
/// <summary>Static helpers emitted by the Vela SQLite core module.</summary>
public static class VelaSqlite
{
    /// <summary>Opens a SQLite database.</summary>
    public static VelaSqliteDatabase Open(string path) => VelaSqliteDatabase.Open(path);

    /// <summary>Executes non-query SQL.</summary>
    public static int Execute(VelaSqliteDatabase database, string sql)
    {
    ...
    public static string Query(VelaSqliteDatabase database, string sql)
    {
        ArgumentNullException.ThrowIfNull(database);
    ...
    public static void Migrate(VelaSqliteDatabase database, string migrationsDirectory)
    {
        ArgumentNullException.ThrowIfNull(database);
    ...
    public static void Close(VelaSqliteDatabase database)
    {
        ArgumentNullException.ThrowIfNull(database);
        database.Close();
    }
...
internal static class VelaDbMigrations
{
    ...
    internal static void Apply(SqliteConnection connection, string migrationsDirectory)
    {
...
{
    internal static string FormatRows(SqliteDataReader reader)
    {
